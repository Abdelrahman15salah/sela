...
const ProductPage = () => {
    ...
    if (error || !product) {
        return (
            <div className="min-h-screen flex justify-center items-center bg-slate-50">
                <div className="text-center py-20 text-red-500 bg-red-50 rounded-2xl px-12">
    ...
    // Strip excessive HTML for meta description if any
    ...
    return (
        <div className="animate-fade-in bg-slate-50 min-h-screen pb-20">
            {/* Dynamic SEO Meta Tags generated for this product */}
            <Helmet>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
                <button
                    type="button"
                    onClick={() => navigate(-1)}
                    className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-brand-600 mb-8 transition-colors"
                >
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                    <div className="flex flex-col md:flex-row gap-8 lg:gap-16">
                        ...
                        {/* Image Gallery */}
                        <div className="bg-slate-50 p-6 md:p-16 flex items-center justify-center min-h-[300px] md:min-h-[400px] w-full md:w-1/2 rounded-t-3xl md:rounded-l-3xl md:rounded-tr-none">
                            <img
                                src={mainImage}
                                alt={product.title}
                                onError={handleImageError}
                                className="w-full h-auto max-w-sm md:max-w-md object-contain mix-blend-multiply drop-shadow-lg animate-float"
                            />
                        </div>
                        ...
                        <div className="p-6 md:p-12 md:pl-0 flex flex-col justify-center w-full md:w-1/2">
                            {categoryName && (
                                <div className="mb-4">
                                    <span className="text-brand-600 text-xs md:text-sm font-medium tracking-widest uppercase bg-brand-50 px-3 py-1 rounded-full inline-block">
                                        {categoryName}
                                    </span>
                                </div>
                            )}

                            <h1 className="text-2xl md:text-4xl font-serif text-dark-900 mb-4 md:mb-6 leading-tight">
                                {product.title}
                            </h1>
                                ...
                                {onSale && saleDisplay ? (
                                    <div className="flex items-baseline gap-2 md:gap-3 flex-wrap">
                                        ...
                                        {typeof product.salePercentage === 'number' && product.salePercentage > 0 && (
                                            <span className="inline-flex items-center rounded-full bg-rose-50 px-2 py-1 md:px-3 text-xs font-semibold text-rose-700">
                                                -{product.salePercentage}%
                                            </span>
                                        )}
                                    </div>
                                ) : (
                                    <div className="text-2xl md:text-3xl font-light text-dark-900">
                                        {priceDisplay}
                                    </div>
                                )}
                            </div>

                            <div className="prose prose-stone mb-8 md:mb-10 text-sm md:text-base text-slate-600">
